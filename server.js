...
const remove = (name) =>
  pool.query(`DELETE FROM leaderboard WHERE username = '${name}'`);
...
pool.query("SELECT * FROM leaderboard;", (err, res) => {
  ...
  pool.query("SELECT * FROM state;", (err, res) => {
    ...
    router.get("/api/press/:name", (ctx, next) => {
      ...
      if (!state.scores.find((e) => e.username === name)) {
        // new person
        ...
        state.scores = state.scores.map((e) =>
          e.username === name ? { username: e.username, score: score } : e
        );
        pool.query(
          `UPDATE leaderboard SET score = ${score} WHERE username = '${name}'`
        );
      }
